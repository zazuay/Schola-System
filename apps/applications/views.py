    ...
    if request.method == 'POST':
        try:
            with transaction.atomic():
                Application.objects.create(user=request.user, scholarship=scholarship)
            messages.success(request, 'Application submitted!')
            return redirect('applications:status')
        except IntegrityError:
            messages.error(request, 'You have already applied for this scholarship.')
    return render(request, 'applications/apply_confirm.html', {'scholarship': scholarship})
...
def upload_view(request, application_pk):
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        return redirect('applications:upload', application_pk=application.pk)
    documents = application.documents.all()
    return render(request, 'applications/upload.html', {
        'form': form, 'application': application, 'documents': documents
    })


@student_required
def status_view(request):
    """Student: view all their applications and statuses."""
    applications = Application.objects.filter(user=request.user).select_related('scholarship')
    return render(request, 'applications/status.html', {'applications': applications})
...
def applicant_list(request):
    ...
    applications = Application.objects.select_related('user', 'scholarship').order_by('-date_submitted')
    return render(request, 'applications/admin_list.html', {'applications': applications})
