...
def scholarship_list(request):
    ...
    return render(request, 'scholarships/list.html', {'scholarships': qs, 'q': q})
...
def admin_scholarship_list(request):
    ...
    scholarships = Scholarship.objects.filter(created_by=request.user).order_by('-created_at')
    return render(request, 'scholarships/admin_list.html', {'scholarships': scholarships})
...
def scholarship_edit(request, pk):
    ...
    scholarship = get_object_or_404(Scholarship, pk=pk)
    form = ScholarshipForm(request.POST or None, instance=scholarship)
    if request.method == 'POST' and form.is_valid():
        form.save()
        return redirect('scholarships:admin_list')
    return render(request, 'scholarships/form.html', {'form': form, 'action': 'Edit'})


@admin_required
def scholarship_delete(request, pk):
    """Admin: delete scholarship (POST only)."""
    scholarship = get_object_or_404(Scholarship, pk=pk)
